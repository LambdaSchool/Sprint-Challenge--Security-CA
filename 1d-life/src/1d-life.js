...
(function() {
  ...
  function getNewVal(lifeState, x) {
    ...
    if (lifeState[x+1] === undefined) {
      totalState = "" + lifeState[x - 1] + lifeState[x] + 0;
    } else if (lifeState[x-1] === undefined) {
      totalState = "" +  0 + lifeState[x] + lifeState[x + 1];
    } else {
      totalState = "" + lifeState[x - 1] + lifeState[x] + lifeState[x + 1];
    }
    // totalState = totalState.join('');
    let state;
    switch (totalState) {
      case '111':
      case '000':
