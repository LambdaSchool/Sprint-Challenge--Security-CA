(function() {
   ...
   */
  //  binary representation of 126 is 01111110
  //
  function getNewVal(lifeState, x) {
    // !!! IMPLEMENT ME
    let lifeRule = [0, 1, 1, 1, 1, 1, 1, 0]; // Rule 126

    let leftCell = lifeState[x - 1];
    let currentCell = lifeState[x];
    let rightCell = lifeState[x + 1];

    // TODO EDGE CASES (set both cases to lifeRule[7])
    ...
    if (leftCell === 1 && currentCell === 1 && rightCell === 1) {
      ...
      return lifeRule[2];
    } else if (leftCell === 1 && currentCell === 0 && rightCell === 0) {
      return lifeRule[3];
