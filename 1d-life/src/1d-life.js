(function() {
  ...
  function getNewVal(lifeState, x) {
    // !!! IMPLEMENT ME
    //if x is surrounded, return 0
    //if x is surrounded by nothingness, return 0
    // else x = 1, return 1;
    // console.log("LifeState: ", lifeState[0]);
    ...
    //no life
    if (left && right === 0 && lifeState[x] === 0) {
      return 0;
    }

    //dies of overpopulation
    if (left && right === 1 && lifeState[x] === 1) {
      return 0;
  ...
  const testArr = [0,0,0];
  const testLife = getNewVal(testArr, testArr[1]);
  console.log("getNewVal:", testLife);
